...
include("includes/test_config.php");
error_reporting(-1);
set_time_limit(-1);
if (isset($_POST['token']))
{
	extract($_REQUEST);
	$consulta = mysqli_query($conexion,"SELECT * FROM aspirantes WHERE MD5(id) = '".$_POST['token']."'");
...
<body style="background: url(http://cert.edu.mx/test_vocacional/assets/img/formato.png) no-repeat top">
	...
	{
		$consulta2 = mysqli_query($conexion,"SELECT SUM(respuesta) AS suma FROM test WHERE id_area = '".$d1['id']."' AND id_aspirante = '".$d['id']."'");
	...
	require_once("../mpdf_6.1.4.0/vendor/autoload.php"); 
