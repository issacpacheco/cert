...
$t = $get->obtener_total_alumnos_inscritos_panel($campus);
$arreglototalinscritos = array("Total" => $t['total'][0]);
$json['totalinscritos'] = $arreglototalinscritos;

$h = $get->obtener_graficas_alumnos_panel($campus);
...
$e = $get->obtener_rango_edad_alumnos_panel($campus);
for($i = 0; $i < count($e['id']); $i++){
    $json['graficasrangoedad']['rangos'][$i]  = array("edad" => $e['edad'][$i], "total" => $e['total'][$i]);
}

$a = $get->obtener_graficas_instituciones_alumnos_panel($campus);
for ($i=0; $i < 10; $i++) { 
...
$g = $get->obtener_generos_alumnos_panel($campus);
